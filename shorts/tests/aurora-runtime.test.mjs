...
test('aurora-explain compiles through the shared HyperFrames entry point', async t => {
  ...
  assert.match(css, /\.ax-connector-layer\{[^}]*z-index:2/);
  assert.match(css, /\.ax-object\{[^}]*z-index:3/);
  assert.match(css, /\.ax-connector-pulse\{[^}]*box-shadow:[^}]*rgba\(84,220,255/);
  assert.match(css, /\.ax-connector-pulse:before\{[^}]*radial-gradient/);
