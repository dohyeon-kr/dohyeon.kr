...
대본·스토리보드 검토용 Markdown과 페이지 수는 본문만 표시한다. `commonPage`로 표시된 공통 CTA는 페이지별 설명에서 생략하고, 매번 사용자에게 다시 검토하도록 제시하지 않는다. 본문 결론은 유지한다. manifest에 이미 포함된 공통 페이지와 최종 렌더의 자동 조립은 유지하며 검토 문서만 필터링한다.
