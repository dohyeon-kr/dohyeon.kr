...
- 정적 사진을 선택하기 전에 같은 장면을 짧은 영상으로 표현했을 때 **행동·공간감·시간의 흐름·정서 변화**가 더 잘 전달되는지 먼저 검토한다. 움직임 자체가 의미라면 사진보다 `backgroundVideo`를 우선한다.
- 특히 사람의 행동, 이동, 공간 분위기, 반복되는 물리적 움직임, 시간 경과, 오프닝/챕터 전환은 영상 후보로 먼저 본다. 비교·인과·구조 설명은 도식, 정확한 사물/인물의 한 순간은 사진, 질문·결론은 타이포가 더 낫다면 그대로 사용한다.
- 일반적인 6~9장 구성에서는 최소 1장, 권장 2~3장을 영상 배경 **후보로 검토**한다. 이는 실제 영상 사용 할당량이 아니다. `videoCatalog`에 의미가 맞는 등록 소스가 없으면 억지로 대체하지 않는다.
- 등록 영상이 없거나 라이선스·길이·구도가 맞지 않으면 사진/도식/타이포로 fallback하고 `visualIntent.strategy.rationale`에 `영상 배경 후보 검토 · 소스 미확보: <원하는 샷>`을 남긴다. 이후 카탈로그 보강 근거로 사용한다.
- 영상 장면은 기존 스키마를 따른다. `backgroundVideo`를 사용하고 `visual.type=none`, `image=null`, `imageQuery=null`, `camera.motion=static`을 유지한다. `strategy.type=video` 같은 미지원 필드를 만들지 않는다.
- `overlayOpacity`는 스키마 범위 0.35~0.85에서 텍스트가 모든 프레임에서 읽히도록 정한다. 배경 영상에는 별도 카메라 이동·줌을 중첩하지 않는다. 2~5초 안에 의미가 성립하는 B-roll을 우선하고, 한 번만 일어나는 행동은 부자연스럽게 loop하지 않는다.
