...
- 연결선과 펄스를 함께 쓰면 연결선을 먼저 완전히 드러낸 다음 최소한 짧은 시차를 두고 펄스가 동일한 중심-중심 경로 위를 지나가게 한다. 펄스가 보이지 않는 선을 먼저 달리거나 선과 다른 궤적을 타면 안 된다.
- 도식에서 설명의 초점이 이동하면 stage 내부만 짧게 zoom/pan해 현재 경로나 노드에 시선을 모은다. 제목과 자막은 고정하고, 보통 1.05~1.10배 범위의 완만한 zoom과 제한된 pan을 사용한다. 연속 경로 설명에서는 다음 경로로 카메라가 부드럽게 이어지고 여유가 있으면 장면 끝에 원래 framing으로 돌아온다.
