...
import type {RenderManifest, RenderScene, SceneLayout, SceneVisual} from './types';
...
const MonochromePhoto: React.FC<{scene: RenderScene; style: React.CSSProperties}> = ({scene, style}) => {
...
const DiagramVisual: React.FC<{visual: SceneVisual; opacity: number}> = ({visual, opacity}) => {
  const motif = visual.motif ?? 'generic';
  const common = {stroke: INK, fill: 'none', strokeWidth: 5, strokeLinecap: 'round' as const, strokeLinejoin: 'round' as const};
  ...
  const body = (() => {
    switch (motif) {
      case 'roi-curve':
        return (
          <>
            ...
            <path d="M110 490 C210 478 280 430 335 350 C395 263 430 155 530 115" {...common} strokeWidth={9} />
            <circle cx="335" cy="350" r="10" fill={INK} />
            <text x="320" y="585" textAnchor="middle" fontSize="27" fill={MUTED}>{visual.xLabel ?? '투입'}</text>
            <text x="35" y="305" textAnchor="middle" fontSize="27" fill={MUTED} transform="rotate(-90 35 305)">{visual.yLabel ?? '효용'}</text>
          </>
        );
      case 'balance-scale':
        return (
      ...
      case 'target':
        return (
          <>
            {[205, 145, 85].map((r) => <circle key={r} cx="315" cy="310" r={r} {...common} />)}
      ...
      case 'arrow-path':
        return (
          <>
            ...
            <path d="M315 505 L270 305 L315 335 L360 305 Z" fill="#c8c8c2" stroke={INK} strokeWidth="4" />
            <text x="315" y="72" textAnchor="middle" fontSize="38" fontWeight="800" fill={INK}>N</text>
          </>
        );
      case 'magnifier':
        return (
      ...
      case 'bookmark-stack':
        return (
          <>
            ...
            {[165, 225, 285, 345].map((y, i) => <line key={y} x1="220" y1={y} x2={i === 3 ? 405 : 455} y2={y} stroke="#9a9a94" strokeWidth="8" strokeLinecap="round" />)}
          </>
        );
      case 'funnel':
        return (
          <>
            ...
            {[135, 205, 275, 345, 415, 485].map((x, i) => <circle key={x} cx={x} cy={80 + (i % 2) * 15} r="15" fill={INK} />)}
            <path d="M315 540 L315 585 M290 560 L315 585 L340 560" {...common} strokeWidth={8} />
          </>
        );
      case 'map-network':
      case 'network': {
        const nodes = [[120,160],[310,105],[500,180],[205,330],[420,350],[300,510]];
        return (
          <>
            {[[0,1],[1,2],[0,3],[1,3],[1,4],[2,4],[3,4],[3,5],[4,5]].map(([a,b]) => <line key={`${a}-${b}`} x1={nodes[a][0]} y1={nodes[a][1]} x2={nodes[b][0]} y2={nodes[b][1]} stroke="#8f8f89" strokeWidth="5" />)}
            {nodes.map(([x,y], i) => <circle key={`${x}-${y}`} cx={x} cy={y} r={i === 1 ? 34 : 24} fill={i === 1 ? INK : PAPER} stroke={INK} strokeWidth="7" />)}
          </>
        );
      }
      case 'fork-road':
      ...
      case 'ranked-list':
        return (
          <>
            {[1,2,3].map((n, i) => (
              <g key={n}>
                <text x="105" y={175 + i * 145} fontSize="68" fontWeight="900" fill={i === 0 ? INK : '#a6a69f'}>{String(n).padStart(2, '0')}</text>
                <line x1="220" y1={150 + i * 145} x2={530 - i * 50} y2={150 + i * 145} stroke={i === 0 ? INK : '#aaa9a3'} strokeWidth="12" strokeLinecap="round" />
              </g>
            ))}
          </>
        );
      case 'ladder':
        return (
          <>
            ...
            {[450,360,270,180].map((y, i) => <line key={y} x1={210 + i * 20} y1={y} x2={410 + i * 20} y2={y} {...common} strokeWidth={10} />)}
      ...
      default:
        return (
          <>
            <circle cx="315" cy="315" r="210" stroke="#b9b9b2" fill="none" strokeWidth="4" strokeDasharray="16 18" />
            <circle cx="315" cy="315" r="110" stroke={INK} fill="none" strokeWidth="8" />
            <circle cx="315" cy="315" r="22" fill={INK} />
            <text x="315" y="610" textAnchor="middle" fontSize="28" fill={MUTED}>{motif}</text>
          </>
        );
    }
  })();
...
const CompareVisual: React.FC<{scene: RenderScene; opacity: number; versus: boolean}> = ({scene, opacity, versus}) => (
  <div
    style={{
      position: 'absolute',
      top: versus ? 315 : 250,
  ...
  >
    ...
    {versus ? (
      <div style={{position: 'absolute', left: '50%', top: 210, transform: 'translate(-50%, -50%)', fontSize: 48, fontWeight: 900, background: PAPER, padding: '14px 18px'}}>VS</div>
    ) : null}
  </div>
);
  ...
  return (
    <div
      style={{
        ...
        borderRadius: 8,
        background: fullBleed ? 'rgba(17,17,15,.88)' : 'rgba(17,17,15,.84)',
        color: PAPER,
        fontSize: 39,
        fontWeight: 700,
        lineHeight: 1.28,
...
const SceneFrame: React.FC<{
  ...
  const frame = useCurrentFrame();
  const entrance = interpolate(frame, [0, 12], [0, 1], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'});
  const textY = interpolate(frame, [0, 14], [34, 0], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'});
  ...
  const headerColor = fullBleed ? '#f4f4f1' : INK;
  ...
  const photo = (() => {
    ...
    if (layout === 'photo-full-bleed') {
      return (
        <>
          <MonochromePhoto scene={scene} style={{position: 'absolute', inset: 0}} />
          <div style={{position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(8,8,8,.08) 10%, rgba(8,8,8,.16) 45%, rgba(8,8,8,.76) 100%)'}} />
        </>
      );
    }
    if (layout === 'photo-split-left') {
      return <MonochromePhoto scene={scene} style={{position: 'absolute', left: 0, top: 0, width: 500, height: 1920}} />;
    }
    if (layout === 'photo-strip') {
      return <MonochromePhoto scene={scene} style={{position: 'absolute', left: 82, top: 430, width: 916, height: 520}} />;
    }
    return <MonochromePhoto scene={scene} style={{position: 'absolute', top: 205, right: 72, width: 700, height: 760}} />;
  })();
  ...
  const diagram = visualAnchor && !isCompare ? (
    <div
      style={{
        position: 'absolute',
        top: layout === 'diagram-centered' ? 245 : 315,
        right: layout === 'diagram-centered' ? 170 : 70,
        width: layout === 'diagram-centered' ? 740 : 520,
        height: layout === 'diagram-centered' ? 740 : 520,
        opacity: entrance,
      }}
    >
      {visual.type === 'number' ? (
        <div style={{width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 260, fontWeight: 900, letterSpacing: '-0.08em'}}>{visual.value ?? String(index + 1).padStart(2, '0')}</div>
      ) : (
        <DiagramVisual visual={visual} opacity={entrance} />
      )}
    </div>
  ) : null;

  const textStyle: React.CSSProperties = (() => {
    if (layout === 'photo-full-bleed') return {left: 82, bottom: 250, width: 880, color: PAPER};
    if (layout === 'photo-split-left') return {left: 555, bottom: 250, width: 445};
    if (layout === 'statement-giant') return {left: 82, top: 370, width: 910};
    if (layout === 'diagram-centered') return {left: 82, bottom: 205, width: 900};
    if (layout === 'symbol-right') return {left: 82, bottom: 300, width: 600};
    if (layout === 'outro-minimal') return {left: 82, bottom: 390, width: 900};
    if (isCompare) return {left: 82, bottom: 205, width: 900};
    return {left: 82, bottom: 230, width: 820};
  })();

  const headlineBase = layout === 'statement-giant' ? 116 : layout === 'photo-split-left' ? 72 : layout === 'outro-minimal' ? 102 : 90;

  return (
    <AbsoluteFill style={{background: PAPER, color: INK, fontFamily: 'Pretendard, Arial, sans-serif', overflow: 'hidden'}}>
      ...
      {photo}
      {diagram}
      {layout === 'compare-columns' ? <CompareVisual scene={scene} opacity={entrance} versus={false} /> : null}
      {layout === 'compare-versus' ? <CompareVisual scene={scene} opacity={entrance} versus /> : null}

      {!photo && !diagram && !isCompare && layout !== 'statement-giant' && layout !== 'outro-minimal' ? (
        <div style={{position: 'absolute', top: 270, right: 94, width: 640, height: 620, borderTop: `4px solid ${INK}`, borderBottom: '1px solid #9b9b96', opacity: entrance}}>
          <div style={{position: 'absolute', right: 0, bottom: 34, fontSize: 160, lineHeight: 0.86, fontWeight: 800, letterSpacing: '-0.07em', color: '#deded9'}}>{String(index + 1).padStart(2, '0')}</div>
        </div>
      ) : null}

      <div style={{position: 'absolute', top: 76, left: 82, display: 'flex', gap: 18, alignItems: 'center', fontSize: 22, letterSpacing: '0.02em', color: secondaryColor, opacity: 0.9, zIndex: 10}}>
        <strong style={{color: headerColor}}>dohyeon.kr</strong><span>•</span><span>{sourceTitle}</span>
      </div>
      <div style={{position: 'absolute', top: 78, right: 82, fontSize: 20, color: secondaryColor, letterSpacing: '0.08em', zIndex: 10}}>{String(index + 1).padStart(2, '0')} / {String(total).padStart(2, '0')}</div>
      ...
      >
        <div style={{fontSize: fontSizeFor(scene.headline, headlineBase), fontWeight: 800, lineHeight: 1.06, letterSpacing: '-0.055em', whiteSpace: 'pre-wrap', wordBreak: 'keep-all', textShadow: fullBleed ? '0 3px 24px rgba(0,0,0,.24)' : 'none'}}>
          {scene.headline}
        </div>
        {scene.subline ? (
          <div style={{marginTop: 30, maxWidth: 780, fontSize: layout === 'photo-split-left' ? 27 : 30, lineHeight: 1.42, letterSpacing: '-0.025em', color: fullBleed ? 'rgba(244,244,241,.8)' : '#575752', whiteSpace: 'pre-wrap', wordBreak: 'keep-all'}}>
            {scene.subline}
          </div>
        ) : null}
      </div>
      ...
      <div style={{position: 'absolute', bottom: 78, left: 82, width: 916, height: 2, background: fullBleed ? 'rgba(244,244,241,.25)' : '#d0d0cb', zIndex: 20}}>
        <div style={{width: `${((index + 1) / total) * 100}%`, height: '100%', background: fullBleed ? PAPER : INK}} />
      </div>
    </AbsoluteFill>
  );
};
