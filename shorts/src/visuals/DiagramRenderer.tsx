...
export const DiagramRenderer: React.FC<Props> = ({spec: input, strict = false, failEngine, ...rest}) => {
  ...
  useNotebookAssets(Boolean(spec.notebook));
  ...
  return <div style={{position: 'relative', width: '100%', height: '100%'}}>
    ...
    {layer !== 'geometry' && <svg viewBox="0 0 800 560" width="100%" height="100%" style={{position: 'absolute', inset: 0, overflow: 'visible'}}>
      {states.filter(node => node.label).map(node => {
        ...
        return <g key={node.id} transform={`translate(${node.x} ${node.y}) rotate(${node.rotation}) scale(${node.scale})`} opacity={node.opacity}>{node.fill === 'hatch' && <rect x={-node.width * .4} y={label.y - lines.length * label.fontSize * LABEL_LINE_HEIGHT / 2 - 4} width={node.width * .8} height={lines.length * label.fontSize * LABEL_LINE_HEIGHT + 8} fill="#050505" />}<text textAnchor="middle" dominantBaseline="central" fill={node.shape !== 'text' && node.fill === 'white' ? '#050505' : '#fff'} fontFamily={spec.notebook ? NOTEBOOK_FONT : "Pretendard, sans-serif"} fontSize={label.fontSize} fontWeight={800}>{lines.map((text, i) => <tspan key={i} x={0} y={label.y + (i - (lines.length - 1) / 2) * label.fontSize * LABEL_LINE_HEIGHT}>{text}</tspan>)}</text></g>;
      })}
    </svg>}
  </div>;
};
const NodeShape: React.FC<{node: ReturnType<typeof evaluatedDiagramState>[number]; notebook?: boolean}> = ({node, notebook}) => {
  ...
  if (notebook && node.role==='sticker') return <Sprite name={node.stickerAsset??'paper'} x={-node.width/2} y={-node.height/2} width={node.width} height={node.height}/>;
  if (notebook && ['rect','circle','line'].includes(node.shape)) return <>{hatch}<defs><pattern id={`${id}-ink`} patternUnits="userSpaceOnUse" width="24" height="24"><svg width="24" height="24" viewBox="160 145 52 52"><image href={assetUrl("marks.webp")} width="1254" height="1254"/></svg></pattern></defs><path d={sketchPath(node)} fill={node.shape==='line'?'none':node.fill==='hatch'?fill:notebookFill(node)} stroke={`url(#${id}-ink)`} strokeWidth={5} strokeLinejoin="round" strokeDasharray={props.strokeDasharray}/></>;
  ...
  return <svg viewBox="0 0 800 560" width="100%" height="100%" role="img" aria-label={spec.description}>
    ...
    {evaluatedDiagramState(spec, frame / Math.max(1, durationInFrames - 1)).map((node) => {
      ...
      return <g key={node.id} transform={`translate(${node.x} ${node.y}) rotate(${node.rotation}) scale(${node.scale})`} opacity={node.opacity}>
        ...
        {node.label && <text textAnchor="middle" dominantBaseline="central" fill={node.shape !== 'text' && node.fill === 'white' ? '#050505' : '#fff'} fontFamily={spec.notebook ? NOTEBOOK_FONT : "Pretendard, sans-serif"} fontSize={label.fontSize} fontWeight={800}>{lines.map((text, i) => <tspan key={i} x={0} y={label.y + (i - (lines.length - 1) / 2) * label.fontSize * LABEL_LINE_HEIGHT}>{text}</tspan>)}</text>}
      </g>;
    })}
  </svg>;
};
