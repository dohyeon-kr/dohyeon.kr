...
const units = (text: string) => [...text].reduce((n, c) => n + (/\s/.test(c) ? .35 : /[\x00-\x7f]/.test(c) ? .62 : 1), 0);
  ...
  for (let size = preferred; size >= 18; size--) {
    ...
    for (const paragraph of text.split('\n')) {
      ...
      for (const word of paragraph.split(/\s+/).filter(Boolean)) {
        ...
        if (line && units(next) * size > width) {lines.push(line); line = '';}
        // Split only a token that cannot fit even on a fresh line.
        for (const char of (line ? ` ${word}` : word)) {
          if (units(line + char) * size > width) {lines.push(line); line = '';}
          line += char;
        }
      }
      lines.push(line);
