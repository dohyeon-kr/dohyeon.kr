...
export const NotebookScene: React.FC<{scene:RenderScene; layer:SceneLayer; durationInFrames:number}> = ({scene,layer,durationInFrames}) => {
  ...
  const title=scene.headline ? fitCopy(scene.headline,640,120,52,1.2) : null;
  return <AbsoluteFill ref={root} style={{background:layer==='visual'?'#101114':undefined,fontFamily:'Pretendard, sans-serif',color:'#fff'}}>
    ...
    {layer==='text' && <>
      {title && <div style={{position:'absolute',left:90,top:230,width:700,height:140,opacity:Math.min(1,frame/6),transform:`translateY(${Math.max(0,1-frame/8)*-6}px)`}}>
        <svg width="700" height="140" style={{position:'absolute',inset:0}} aria-hidden="true"><g transform="rotate(-1 350 65)"><Sprite name="tape" x={0} y={0} width={700} height={140}/></g></svg>
        <div data-layout-text="headline" style={{position:'relative',padding:'12px 26px',fontFamily:NOTEBOOK_FONT,fontSize:title.fontSize,lineHeight:1.2,fontWeight:400,whiteSpace:'pre-wrap',color:'#eef5ff'}}>{title.text}</div>
      </div>}
