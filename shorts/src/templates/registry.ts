...
사진·도식이 없는 장면은 짧은 의미 단위로 headline을 줄바꿈하고 큰 타이포를 화면 중앙에 배치한다. 100px 이상 큰 제목에는 도식과 같은 약한 스크리블을 0.1초 간격으로 적용한다. 글자 위치·줄바꿈은 고정하며 작은 자막·보조 문구·도식 라벨은 선명하게 유지한다. 워크플로우의 변경 전후·의존 관계·요청 흐름은 도식으로 적극 설명하되 원문 밖 경험·수치·운영 규칙을 만들지 않는다.
