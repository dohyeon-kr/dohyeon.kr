// Source pages and Pexels license checked 2026-09-16. These are contextual stock
// illustrations, not photographs of the author or the events in the narration.
export function curatedPhoto(query) {
  let entry;
  if (/lottery|lotto|로또/i.test(query)) {
    entry = [8266775, '손에 든 로또 티켓', 'Erik Mclean', 'a-person-holding-lottery-tickets-8266775'];
  } else if (/open.*door|door.*light|doorway|열린.*문/i.test(query)) {
    entry = [4132661, '어두운 방의 살짝 열린 문', 'Sam J', 'slightly-opened-door-in-dark-room-4132661'];
  } else if (/tired|friction|conflict|empty.*(office|room)|회의실/i.test(query)) {
  ...
  } else if (/team|collaborat|meeting|whiteboard|conversation|people.*talk|협업|대화/i.test(query)) {
