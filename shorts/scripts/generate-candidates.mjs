...
const SceneSchema = z.object({
  ...
  diagramSpec: DiagramSpecSchema.extend({notebook: DiagramSpecSchema.shape.notebook.unwrap().default(null), events: z.array(GeneratedDiagramEventSchema).max(120), physics: DiagramSpecSchema.shape.physics.unwrap(), nodes: z.array(DiagramSpecSchema.shape.nodes.element.extend({role: DiagramSpecSchema.shape.nodes.element.shape.role.unwrap().default(null), connector: DiagramSpecSchema.shape.nodes.element.shape.connector.unwrap(), strokeStyle: z.enum(['solid', 'dashed']).nullable()})).min(1).max(40)}).nullable(),
...
오답노트 테마는 명시적으로 요청된 경우 diagramSpec.notebook={theme:'error-notebook',maxStickerOverlap:0.2}로 선택한다. 보통 notebook=null, node.role=null이다. 스티커만 rect에 role='sticker'를 지정하고 의미 없는 장식은 넣지 않는다. 다른 도형을 sticker로 바꿔 검증을 우회하지 않는다. 스티커 겹침은 회전·확대 및 선 두께를 포함한 자기 footprint 대비 다른 보이는 도형 footprint와 겹친 합집합 면적 비율이다. 기본 20%, 허용 상한 40%이며 여러 작은 겹침도 합쳐 검사한다. 종이 배경은 엔진이 별도 배경으로 그린다. 라벨·자막·안전영역은 완화하지 않으며 채워진 스티커와 선의 교차만 면적 예산 안에서 허용한다. 낙서 선은 ID 기반으로 고정되어 프레임마다 떨리지 않고 기존 events/connector로 제어한다. 이 테마의 자막은 고정 46px·최대 두 줄이므로 의미 단위를 짧게 작성한다. presenterOverlay.frame='torn-paper-blue'는 수동 manifest용 찢어진 파란 종이 프레임이며 기존 입/고개 트랙과 CTA 숨김을 보존한다.
