...
const SceneSchema = z.object({
  ...
  diagramSpec: DiagramSpecSchema.extend({notebook: DiagramSpecSchema.shape.notebook.unwrap().default(null), events: z.array(GeneratedDiagramEventSchema).max(120), physics: DiagramSpecSchema.shape.physics.unwrap(), nodes: z.array(DiagramSpecSchema.shape.nodes.element.extend({role: DiagramSpecSchema.shape.nodes.element.shape.role.unwrap().default(null), stickerAsset: DiagramSpecSchema.shape.nodes.element.shape.stickerAsset.unwrap().default(null), stickerAttachment: DiagramSpecSchema.shape.nodes.element.shape.stickerAttachment.unwrap().default(null), connector: DiagramSpecSchema.shape.nodes.element.shape.connector.unwrap(), strokeStyle: z.enum(['solid', 'dashed']).nullable()})).min(1).max(40)}).nullable(),
...
오답노트 테마의 제목 오프닝을 요청하면 첫 장면은 layout=notebook-title, kind=hero, visual.type=photo, diagramSpec=null로 만든다. 제목은 84px 최대 세 줄 분량으로 짧게 쓰며 마지막 줄을 파란 강조로 사용한다. subline은 짧은 Pretendard 메모이고 내레이션 전체를 반복하지 않는다. 실제 주제를 보여주는 사진을 검색하여 합성하며 이미지 밑줄과 모서리를 고정하는 테이프는 엔진 애셋으로 처리한다. 사진은 넓은 직사각 인화 사진으로 보여준다. 오프닝은 3~4초 안팎이며 다음 장면부터 본문으로 들어간다. 제목 페이지는 본문의 첫 장면이며 별도 썸네일이 아니다.
오답노트 테마는 명시적으로 요청된 경우 diagramSpec.notebook={theme:'error-notebook',maxStickerOverlap:0.2}로 선택한다. 보통 notebook=null, node.role=null이다. 스티커만 rect에 role='sticker'를 지정하고 의미 없는 장식은 넣지 않는다. 다른 도형을 sticker로 바꿔 검증을 우회하지 않는다. 스티커 겹침은 회전·확대 및 선 두께를 포함한 자기 footprint 대비 다른 보이는 도형 footprint와 겹친 합집합 면적 비율이다. 기본 20%, 허용 상한 40%이며 여러 작은 겹침도 합쳐 검사한다. 종이 배경은 엔진이 별도 배경으로 그린다. 라벨·자막·안전영역은 완화하지 않으며 채워진 스티커와 선의 교차만 면적 예산 안에서 허용한다. 낙서 선은 ID 기반으로 고정되어 프레임마다 떨리지 않고 기존 events/connector로 제어한다. 이 테마는 headline에 페이지별 소제목이 아닌 전체 주제 제목을 쓴다. 엔진은 source.title을 좌측 상단에 모든 본문 페이지에서 동일하게 표시한다. 종이 지면은 화면 전체이며 도식을 작은 종이 패널 안에 가두지 않는다. 내레이션은 존댓말의 1인칭 회고를 허용하고 자기비하·비하 속어는 행동에 대한 구체적인 표현으로 순화한다. 전체 화면은 고정 종이 질감을 은은하게 합성하고 자막 줄마다 받침 여백을 둔 파란 공책 선을 표시한다. 노트 자막은 beats.keyword를 이미지 마커로 강조하므로 한 줄에 들어갈 짧은 핵심어를 지정한다. 글자 확대 없이 마커만 등장한다. 이 테마의 자막은 고정 46px·최대 두 줄이므로 의미 단위를 짧게 작성한다. 프레젠터는 기존 원형을 사용한다. 제목·도식 라벨·스티커 메모·자막 모두 Pretendard 굵은 글씨를 쓴다. 손글씨는 사용하지 않는다. stickerAsset에는 paper/blue/tape/check/star/underline을 선택할 수 있다. 노트 테마 장면마다 의미를 강조하는 이미지 스티커 2~3개를 필요에 맞게 배치하되 장수별 할당량을 채우지 않는다. stickerAttachment={target:객체ID,preset:edge-note|tape|badge}는 선택사항이다. edge-note는 paper/blue만 대상으로 8~20%, tape는 tape만 35~65%, badge는 check/star만 5~15% 걸친다. 비율은 스티커 footprint 기준이다. 지정 대상 외 겹침은 기존 전역 상한을 유지하고 텍스트 보호는 예외가 없다. 완전히 떨어진 메모는 attachment=null로 둔다. 상세 용례는 notebook-stickers.md를 따른다. 종이와 잉크는 실제 이미지 질감을 합성하며 선 경로는 코드로 제어한다.
