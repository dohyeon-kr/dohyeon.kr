...
- Age uses Ghost's `updated_at`, not `created_at` or `published_at`. Saving an update refreshes the bonus, including small edits that change this timestamp.
- Reactions are cached on the first request of each KST hour; time since the latest update is evaluated at the hour boundary, clamped to zero for posts updated during that hour. A service restart refreshes the snapshot. Existing pages keep their order until reload.
- Ties prefer the more recent update, then the slug alphabetically. Posts with zero views remain eligible.
...
Validation: `python -m unittest discover -s services/visit-counter -v`; after deployment, `python scripts/check-live-carousel.py` verifies live assets, update metadata and descending scores.
